...
/**
 * Retrieves raw input data for a given HRAWINPUT device handle.
 * If successful, returns a RAWINPUT structure containing the
 * device's input state.
 */
pub fn get_raw_input_data(handle: HRAWINPUT) -> Option<RAWINPUT> {
    ...
    //Possible error states for GetRawInputData()
    //are -1;
    //0 indicates null data was returned
    if status == UINT::max_value() || status == 0 {
        return None;
    }
